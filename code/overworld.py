...
class Overworld:
    def __init__(self, current_level, screen_surface, create_level, change_game_level, total_game_time, game_data, user_login, user_data):
        # setup
        ...
        # side score bar
        self.my_stars_score = Stars_Score(self.display_surface, self.user_data)
    ...
    def create_nodes(self):
        # here in this section we need mix data from two different sources: my_game_data and user_data dictionary's
        # this need because of user_data is a controlling data which can change , my_game_data doesn't change

        my_level_stars = self.user_data['levels_stars']

        for level in my_level_stars:
            if self.my_stars_score.my_score >= self.my_game_data[level]['stars_for_unlock']:
                node_data = self.my_game_data[level]
                node_stars = my_level_stars[level]
                node_object = Node(node_data, node_stars, 'available', self.speed, self.display_surface)
            else:
                node_data = self.my_game_data[level]
                node_stars = my_level_stars[level]
                node_object = Node(node_data, node_stars, 'locked', self.speed, self.display_surface)
...
#######################################################################################################################
class Stars_Score:
    def __init__(self, screen: pygame.surface.Surface, my_user_data):
        self.display_surface = screen
        self.my_user_data = my_user_data
        self.my_score = 0
        self.max_stars_qtty = len(self.my_user_data['levels_stars'])*3
    ...
    def update_score(self):
        my_levels = self.my_user_data['levels_stars']
        for level in my_levels:
            self.my_score += my_levels[level]
